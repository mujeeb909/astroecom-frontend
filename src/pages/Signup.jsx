...
export const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMsg(err.data?.message || 'Failed to create account. Please try again.');
    }
  };
  ...
  return (
    <AuthLayout
      ...
      subtitle="Join 1,000+ brands using AstroEcom"
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <p className="text-center text-body-sm text-text-secondary-light dark:text-text-secondary-dark font-medium pt-4">
          Already using AstoEcom?{' '}
          <Link to="/login" className="text-primary hover:text-primary-600 transition-colors font-extrabold underline-offset-4 hover:underline">
            Sign in
          </Link>
        </p>

        <p className="text-[10px] text-center text-text-secondary-light dark:text-text-secondary-dark opacity-50 px-6">
